	...
	(
		...
		return {
			async handleMessage({ source: tpa, origin, message }) {
				...
				const appDefinitionId = (isTpaWorker
					? workerDetails!.appDefinitionId
					: tpaCompData?.appDefinitionId) as string
				...
				const extras: TpaHandlerExtras = {
					...
					appDefinitionId,
					tpaCompData,
					appClientSpecMapData: { appDefinitionId, ...appsClientSpecMapData[appDefinitionId] },
					originCompId,
