...
export function isPreOrder(product, variant) {
    if (!product && !variant) {
        return false;
    }
    if (variant) {
        return variant.availableForPreOrder;
    }
    if (product.isManageProductItems) {
        var isAllVariantsPreOrder = product.productItemsPreOrderAvailability === IProductPreOrderAvailability.ALL_VARIANTS;
        var isSomeVariantsPreOrder = product.productItemsPreOrderAvailability === IProductPreOrderAvailability.SOME_VARIANTS;
        return isAllVariantsPreOrder || (isSomeVariantsPreOrder && !product.isInStock);
    }
    return product.inventory.availableForPreOrder;
}
...
export function validateUserInputs(product, userInputs, shouldUseItemLevelAvailablePreOrder) {
    var errors = userInputsFactory();
    var quantityRange = QuantityCalculator.getQuantitiesRange(product, userInputs.selection, shouldUseItemLevelAvailablePreOrder);
...
export function hasAutomaticDiscount(product) {
    var _a;
    return Boolean((_a = product.itemDiscount) === null || _a === void 0 ? void 0 : _a.priceAfterDiscount);
}
...
function hasManualDiscount(product) {
    var _a;
    return ((_a = product.discount) === null || _a === void 0 ? void 0 : _a.value) > 0 && Boolean(product.formattedComparePrice);
}
export function hasAnyDiscount(product) {
    return hasManualDiscount(product) || hasAutomaticDiscount(product);
}
export function formattedPrimaryPrice(product) {
    var isAutomaticDiscountExist = hasAutomaticDiscount(product);
    var isManualDiscountExist = hasManualDiscount(product);
    if (isAutomaticDiscountExist) {
        return product.itemDiscount.priceAfterDiscount;
    } else if (isManualDiscountExist) {
        return product.formattedComparePrice;
    }
    return product.formattedPrice;
}
export function formattedSecondaryPrice(product) {
    var isAutomaticDiscountExist = hasAutomaticDiscount(product);
    var isManualDiscountExist = hasManualDiscount(product);
    if (isAutomaticDiscountExist && isManualDiscountExist) {
        return product.formattedComparePrice;
    } else if (isManualDiscountExist || isAutomaticDiscountExist) {
        return product.formattedPrice;
    }
    return undefined;
}
...
export function variantsCount(product) {
    var _a, _b;
    if (((_a = product.productItemsSummary) === null || _a === void 0 ? void 0 : _a.productItemsCount) > 0) {
        return product.productItemsSummary.productItemsCount;
    }
    if (((_b = product.options) === null || _b === void 0 ? void 0 : _b.length) > 0) {
        return product.options.map(function(o) {
            return o.selections.length;
