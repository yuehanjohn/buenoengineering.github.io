...
import {
    getProGalleryStyles
} from '../utils/proGalleryStyleBuilder';
import {
    designConsts
} from '../utils/designConsts';
...
function designer_external_disableSocialIfNotSlideshow(master, galleryLayout) {
    var isSlideshow = GALLERY_v4_CONSTS.isLayout('SLIDESHOW')({
...
function designer_internal_sliderOrThumbnail_cancelMasterPaddingBecauseItIsHardCodedInCSS(master, galleryLayout) {
    var isSlider = GALLERY_v4_CONSTS.isLayout('SLIDER')({
        galleryLayout: galleryLayout
    });
    var isThumbnail = GALLERY_v4_CONSTS.isLayout('THUMBNAIL')({
        galleryLayout: galleryLayout
    });
...
function deleteFieldFromAllSections(master, fieldName) {
    Object.keys(master.order).forEach(function(section) {
        return master.order[section] = master.order[section].filter(function(iType) {
...
function getSection(iType, master) {
    ...
    var middle = "middle";
    var bottom = "bottom";
    return [top, middle, bottom].find(function(sName) {
        return master.order[sName].includes(iType);
    }) || top; // if nothing is found assuming top
}
...
function addNewInfoElementsConfig(bla, enabled, order) {
    ...
    var res = __assign(__assign({}, bla), {
        newInfoElementsConfig: config
    });
...
function disableTexts(master) {
    ...
    infoTypes.forEach(function(iType) {
        return master.enabled[iType] = false;
    });
}
...
export {
    convertWixStyleParamsToMasterInfoElementsOptions,
