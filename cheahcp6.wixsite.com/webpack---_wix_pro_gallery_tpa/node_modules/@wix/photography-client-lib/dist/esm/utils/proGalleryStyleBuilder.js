...
import {
    isHorizontalInfoCompatibleLayout
} from './proGalleryStyleBuilderUtils';
import {
    experimentsWrapper
...
var mobileSpecificDefaultStyles = {
...
var baseDefaultStyles = mergeNestedObjects(coreOptions, {
    ...
    itemClick: 'expand',
    fixedColumns: 0,
    scrollDirection: GALLERY_v4_CONSTS.scrollDirection.VERTICAL,
    showArrows: true,
    hasThumbnails: false,
    ...
    imageLoadingMode: GALLERY_v4_CONSTS.loadingMode.COLOR,
    imageLoadingWithColorMode: GALLERY_v4_CONSTS.loadingWithColorMode.PICKED_COLOR,
    ...
    previewHover: false,
    calculateTextBoxHeightMode: GALLERY_v4_CONSTS.textBoxWidthCalculationOptions.AUTOMATIC,
    slideAnimation: GALLERY_v4_CONSTS.slideAnimations.SCROLL,
    jsonStyleParams: '',
    designedPresetId: -1,
    galleryLayoutType: '',
    ...
    overlayType: 0,
    overlayPosition: GALLERY_v4_CONSTS.overlayPositions.BOTTOM,
    overlaySize: 100,
    overlayPadding: 0,
    overlaySizeType: GALLERY_v4_CONSTS.overlaySizeType.PERCENT,
    arrowsVerticalPosition: GALLERY_v4_CONSTS.arrowsVerticalPosition.ITEM_CENTER,
    textBoxWidthPercent: 50,
    textBoxWidth: 200,
    calculateTextBoxWidthMode: GALLERY_v4_CONSTS.textBoxWidthCalculationOptions.PERCENT,
    customArrowsSvg: '',
...
var convertToMobileSettingIfNeeded = function(isMobile, styles) {
    var userSavedMobileStyles = {};
    if (isMobile) {
        Object.keys(styles).forEach(function(val) {
            if (val.startsWith('m_')) {
                var key = val.slice(2);
                styles[key] = styles[val];
                userSavedMobileStyles[key] = styles[val];
            }
        });
    }
    return {
        styles: styles,
        userSavedMobileStyles: userSavedMobileStyles
    };
};
export var getProGalleryStylesImp = function(wixStyles, gotStyleParams, options) {
    var isMobile = options.isMobile,
        ...
        isInEditor = _a === void 0 ? false : _a,
        _b = options.isSsr,
        isSsr = _b === void 0 ? false : _b;
    ...
    stateStyles.useOptimizedImageFormat = wixStyles.useOptimizedImageFormat;
    ...
    wixStyles = mobileConvertedStyles;
    if (String(wixStyles.mobilePanorama) === '1' && isMobile) {
        ...
        stateStyles.scrollDirection = GALLERY_v4_CONSTS.scrollDirection.VERTICAL;
    }
    // behaviour
    ...
    if (canSet('previewHover')) {
        var date = Date.now();
        if (date > wixStyles.previewHover + 2000) {
            stateStyles.previewHover = false;
        } else {
            stateStyles.previewHover = true;
        }
    }
    ...
    if (canSet('allowOverlayGradient')) {
        ...
        if (allowOverlayGradient) {
            stateStyles.overlayBackground = "linear-gradient(".concat(overlayGradientDegrees, "deg,").concat(overlayGradientColor1.value, ",").concat(overlayGradientColor2.value, ")");
        } else {
    ...
    if (stateStyles.imageRatioType === Consts.imageRatioType.RESPONSIVE) {
        stateStyles.gallerySliderImageRatio = "".concat((100 / Number(stateStyles.numberOfDisplayedItems)).toFixed(2), "%/100%");
    ...
    if (canSet('allowFixedGalleryRatio')) {
        if (wixStyles.allowFixedGalleryRatio) {
            if (canSet('fixedGalleryRatio')) {
                stateStyles = assignByString(stateStyles, 'layoutParams_structure_galleryRatio_value', 1 /
                    eval(['16/9', '4/3', '1', '3/4', '9/16'][Number(wixStyles.fixedGalleryRatio)]));
            } else {
                stateStyles = assignByString(stateStyles, 'layoutParams_structure_galleryRatio_value', 1);
            }
        }
    ...
    // };
    // stateStyles.scrollDirection = isHorizontalLayout() ? scrollDirection === GALLERY_v4_CONSTS.scrollDirection.HORIZONTAL : scrollDirection === GALLERY_v4_CONSTS.scrollDirection.VERTICAL;
    // if (stateStyles.scrollDirection === GALLERY_v4_CONSTS.scrollDirection.HORIZONTAL) {
    ...
    var setTitlePlacementIfPossible = function() {
        var _a = GALLERY_v4_CONSTS.placements,
            SHOW_BELOW = _a.SHOW_BELOW,
            SHOW_ON_HOVER = _a.SHOW_ON_HOVER,
            SHOW_ABOVE = _a.SHOW_ABOVE;
        if (canSet('titlePlacement')) {
            stateStyles.titlePlacement = {
                0: SHOW_BELOW,
                2: SHOW_ABOVE,
            }[String(wixStyles.titlePlacement)] || SHOW_ON_HOVER;
        }
    };
    var setCompTitlePlacementIfPossible = function() {
        if (canSet('titlePlacementHorizontallyCompatible', 'titlePlacement')) {
            var _a = GALLERY_v4_CONSTS.placements,
                SHOW_BELOW = _a.SHOW_BELOW,
                SHOW_ON_HOVER = _a.SHOW_ON_HOVER,
                SHOW_ABOVE = _a.SHOW_ABOVE,
                SHOW_ON_THE_LEFT = _a.SHOW_ON_THE_LEFT,
                SHOW_ON_THE_RIGHT = _a.SHOW_ON_THE_RIGHT;
            stateStyles.titlePlacement = {
                    0: SHOW_BELOW,
                    2: SHOW_ABOVE,
                    3: SHOW_ON_THE_LEFT,
                    4: SHOW_ON_THE_RIGHT,
                }[String(wixStyles.titlePlacementHorizontallyCompatible)] ||
                SHOW_ON_HOVER;
        }
    };
    var setCompTitlePlacement = function() {
        // Notice: The order matters here! those setting calls must be applied that way
        if (isMobile) {
            if (!isUndefined(wixStyles['m_titlePlacementHorizontallyCompatible'])) {
                setCompTitlePlacementIfPossible();
            } else if (!isUndefined(wixStyles['m_titlePlacement'])) {
                setTitlePlacementIfPossible();
            } else {
                setCompTitlePlacementIfPossible();
                setTitlePlacementIfPossible();
            }
    ...
    var setTitlePlacementValue = function() {
        isHorizontalInfoCompatibleLayout(__assign(__assign({}, stateStyles), {
                isMobile: isMobile
            })) ?
            setCompTitlePlacement() :
            setTitlePlacementIfPossible();
    };
    // SP set by experiment
    experimentsWrapper.getExperimentBoolean('specs.pro-gallery.horizontalTitlePlacementOptionsViewer') ?
        setTitlePlacementValue() :
        setTitlePlacementIfPossible();
    if (canSet('hoveringBehaviour')) {
        if (String(wixStyles.hoveringBehaviour) === '1') {
            stateStyles.hoveringBehaviour =
                GALLERY_v4_CONSTS.infoBehaviourOnHover.DISAPPEARS;
        } else if (String(wixStyles.hoveringBehaviour) === '2') {
            stateStyles.hoveringBehaviour =
                GALLERY_v4_CONSTS.infoBehaviourOnHover.NO_CHANGE;
        } else {
            // String(wixStyles.hoveringBehaviour) === '0'
            stateStyles.hoveringBehaviour =
                GALLERY_v4_CONSTS.infoBehaviourOnHover.APPEARS;
        }
    }
    if (canSet('overlaySizeType') && String(wixStyles.overlaySizeType) === '1') {
        stateStyles.overlaySizeType = GALLERY_v4_CONSTS.overlaySizeType.PIXEL;
        if (canSet('overlaySizePixel')) {
            stateStyles.overlaySize = Number(wixStyles.overlaySizePixel);
        }
    } else {
        stateStyles.overlaySizeType = GALLERY_v4_CONSTS.overlaySizeType.PERCENT;
    ...
    if (canSet('overlayPosition')) {
        ...
        var _d = GALLERY_v4_CONSTS.overlayPositions,
            LEFT = _d.LEFT,
            RIGHT = _d.RIGHT,
            BOTTOM = _d.BOTTOM,
            TOP = _d.TOP,
    ...
    if (!isUndefined(stateStyles.allowTitle) &&
        ...
        stateStyles.titlePlacement = GALLERY_v4_CONSTS.placements.SHOW_ON_HOVER;
        stateStyles.hoveringBehaviour =
            GALLERY_v4_CONSTS.infoBehaviourOnHover.APPEARS;
    }
    if (canSet('arrowsVerticalPosition')) {
        stateStyles.arrowsVerticalPosition = {
            0: GALLERY_v4_CONSTS.arrowsVerticalPosition.ITEM_CENTER,
            1: GALLERY_v4_CONSTS.arrowsVerticalPosition.IMAGE_CENTER,
            2: GALLERY_v4_CONSTS.arrowsVerticalPosition.INFO_CENTER,
        }[wixStyles.arrowsVerticalPosition];
    } else if (GALLERY_v4_CONSTS.isLayout('SLIDESHOW')(stateStyles)) {
        stateStyles.arrowsVerticalPosition =
            GALLERY_v4_CONSTS.arrowsVerticalPosition.IMAGE_CENTER;
    }
    if (canSet('arrowsType')) {
        var type = {
            0: GALLERY_v4_CONSTS.arrowsType.DEFAULT_ARROW,
            1: GALLERY_v4_CONSTS.arrowsType.ARROW_2,
            2: GALLERY_v4_CONSTS.arrowsType.ARROW_3,
            3: GALLERY_v4_CONSTS.arrowsType.ARROW_4,
            4: GALLERY_v4_CONSTS.arrowsType.ARROW_5,
            5: GALLERY_v4_CONSTS.arrowsType.ARROW_6,
            6: GALLERY_v4_CONSTS.arrowsType.ARROW_7,
        }[wixStyles.arrowsType] || GALLERY_v4_CONSTS.arrowsType.DEFAULT_ARROW;
        stateStyles = assignByString(stateStyles, optionsMap.layoutParams.navigationArrows.type, type);
        // check use of custom arrows
        if ([-1, -2].includes(wixStyles.arrowsType)) {
            stateStyles.shouldUseCustomArrows = true;
            stateStyles.isArrowPrivateMedia = wixStyles.isArrowPrivateMedia;
        }
    }
    if (canSet('arrowsContainerType')) {
        var type = {
                0: GALLERY_v4_CONSTS.arrowsContainerStyleType.NONE,
                1: GALLERY_v4_CONSTS.arrowsContainerStyleType.SHADOW,
                2: GALLERY_v4_CONSTS.arrowsContainerStyleType.BOX,
            }[wixStyles.arrowsContainerType] ||
            GALLERY_v4_CONSTS.arrowsContainerStyleType.SHADOW;
        stateStyles = assignByString(stateStyles, optionsMap.layoutParams.navigationArrows.container.type, type);
    }
    if (canSet('arrowsContainerBackgroundColor')) {
        stateStyles = assignByString(stateStyles, optionsMap.layoutParams.navigationArrows.container.backgroundColor, wixStyles.arrowsContainerBackgroundColor.value);
    }
    if (canSet('arrowsContainerBorderRadius')) {
        stateStyles = assignByString(stateStyles, optionsMap.layoutParams.navigationArrows.container.borderRadius, wixStyles.arrowsContainerBorderRadius);
    }
    if (canSet('customArrowsSvg')) {
        stateStyles.customArrowsSvg = wixStyles.customArrowsSvg;
    }
    if (canSet('customArrowsRotation')) {
        stateStyles.customArrowsRotation = wixStyles.customArrowsRotation;
    }
    if (canSet('scrollAnimation')) {
        ...
        if (shouldNotAllowHorizontalScrollAnimation) {
            stateStyles.scrollAnimation =
                GALLERY_v4_CONSTS.scrollAnimations.NO_EFFECT;
        } else if (String(wixStyles.scrollAnimation) === '1') {
            stateStyles.scrollAnimation = GALLERY_v4_CONSTS.scrollAnimations.FADE_IN;
        } else if (String(wixStyles.scrollAnimation) === '2') {
            stateStyles.scrollAnimation =
                GALLERY_v4_CONSTS.scrollAnimations.GRAYSCALE;
        } else if (String(wixStyles.scrollAnimation) === '3') {
            stateStyles.scrollAnimation = GALLERY_v4_CONSTS.scrollAnimations.SLIDE_UP;
        } else if (String(wixStyles.scrollAnimation) === '4') {
            stateStyles.scrollAnimation = GALLERY_v4_CONSTS.scrollAnimations.EXPAND;
        } else if (String(wixStyles.scrollAnimation) === '5') {
            stateStyles.scrollAnimation = GALLERY_v4_CONSTS.scrollAnimations.SHRINK;
        } else if (String(wixStyles.scrollAnimation) === '6') {
            stateStyles.scrollAnimation = GALLERY_v4_CONSTS.scrollAnimations.ZOOM_OUT;
        } else if (String(wixStyles.scrollAnimation) === '7') {
            stateStyles.scrollAnimation =
                GALLERY_v4_CONSTS.scrollAnimations.ONE_COLOR;
        } else {
    ...
    if (canSet('slideAnimation')) {
        ...
        if (!shouldAllowSlideAnimation) {
            stateStyles.slideAnimation = GALLERY_v4_CONSTS.slideAnimations.SCROLL;
        } else {
            var slideAnimation = String(wixStyles.slideAnimation);
            switch (slideAnimation) {
                case '1':
                    stateStyles.slideAnimation = GALLERY_v4_CONSTS.slideAnimations.FADE;
                    break;
                case '2':
                    stateStyles.slideAnimation = GALLERY_v4_CONSTS.slideAnimations.DECK;
                    break;
                case '0':
                default:
                    stateStyles.slideAnimation = GALLERY_v4_CONSTS.slideAnimations.SCROLL;
    ...
    if (canSet('overlayAnimation')) {
        ...
        var _e = GALLERY_v4_CONSTS.overlayAnimations,
    ...
    if (canSet('imageHoverAnimation')) {
        if (String(wixStyles.imageHoverAnimation) === '1') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.ZOOM_IN;
        } else if (String(wixStyles.imageHoverAnimation) === '2') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.BLUR;
        } else if (String(wixStyles.imageHoverAnimation) === '3') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.GRAYSCALE;
        } else if (String(wixStyles.imageHoverAnimation) === '4') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.SHRINK;
        } else if (String(wixStyles.imageHoverAnimation) === '5') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.INVERT;
        } else if (String(wixStyles.imageHoverAnimation) === '6') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.COLOR_IN;
        } else if (String(wixStyles.imageHoverAnimation) === '7') {
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.DARKENED;
        } else {
            // String(wixStyles.imageHoverAnimation) === '0'
            stateStyles.imageHoverAnimation =
                GALLERY_v4_CONSTS.imageHoverAnimations.NO_EFFECT;
        }
    }
    ...
    if (canSet('imageInfoType')) {
        ...
        if (String(wixStyles.imageInfoType) === '0') {
            selectedImageInfoType = GALLERY_v4_CONSTS.infoType.NO_BACKGROUND;
        } else if (String(wixStyles.imageInfoType) === '1') {
            selectedImageInfoType = GALLERY_v4_CONSTS.infoType.ATTACHED_BACKGROUND;
        } else if (String(wixStyles.imageInfoType) === '2') {
            selectedImageInfoType = GALLERY_v4_CONSTS.infoType.SEPARATED_BACKGROUND;
        } else {
            selectedImageInfoType = GALLERY_v4_CONSTS.infoType.NO_BACKGROUND;
        }
    ...
    if (canSet('autoSlideshowType')) {
        ...
        var shouldAllowContinuousAutoSlide = stateStyles.slideAnimation === GALLERY_v4_CONSTS.slideAnimations.SCROLL;
    ...
    var fullResFeatureEnviromentSupported = !isInEditor && !isSsr;
    var fullResFeatureStyleParamSet = wixStyles.galleryItemResolution !== undefined;
    var shouldUseFullResFeature = fullResFeatureExperimentEnabled &&
        fullResFeatureEnviromentSupported &&
        fullResFeatureStyleParamSet;
    if (shouldUseFullResFeature) {
        stateStyles = assignByString(stateStyles, 'stylingParams_itemResolutionMode', galleryItemResolutionModesMap[wixStyles.galleryItemResolution]);
    }
    if (canSet('videoPlay')) {
    ...
    if (canSet('imageLoadingMode')) {
        if (String(wixStyles.imageLoadingMode) === '1') {
            stateStyles.imageLoadingMode = GALLERY_v4_CONSTS.loadingMode.COLOR;
        } else {
            // String(wixStyles.imageLoadingMode) === '0'
            stateStyles.imageLoadingMode = GALLERY_v4_CONSTS.loadingMode.BLUR;
        }
    }
    if (canSet('imageLoadingWithColorMode')) {
        if (String(wixStyles.imageLoadingWithColorMode) === '1') {
            stateStyles.imageLoadingWithColorMode =
                GALLERY_v4_CONSTS.loadingWithColorMode.MAIN_COLOR;
        } else {
            // String(wixStyles.imageLoadingWithColorMode) === '0'
            stateStyles.imageLoadingWithColorMode =
                GALLERY_v4_CONSTS.loadingWithColorMode.PICKED_COLOR;
        }
    }
    ...
    if (canSet('calculateTextBoxHeightMode')) {
        if (String(wixStyles.calculateTextBoxHeightMode) === '1') {
            stateStyles.calculateTextBoxHeightMode =
                GALLERY_v4_CONSTS.textBoxWidthCalculationOptions.MANUAL;
        } else {
            // String(wixStyles.calculateTextBoxHeightMode) === '0'
            stateStyles.calculateTextBoxHeightMode =
                GALLERY_v4_CONSTS.textBoxWidthCalculationOptions.AUTOMATIC;
        }
    } else {
        stateStyles.calculateTextBoxHeightMode =
            defaultStateStyles.calculateTextBoxHeightMode;
    }
    // Define those only if gallery info on the left \ right
    if (canSet('calculateTextBoxWidthMode')) {
        stateStyles.calculateTextBoxWidthMode = {
            0: GALLERY_v4_CONSTS.textBoxWidthCalculationOptions.PERCENT,
            1: GALLERY_v4_CONSTS.textBoxWidthCalculationOptions.MANUAL,
        }[wixStyles.calculateTextBoxWidthMode];
    }
    if (canSet('textBoxWidthPercent')) {
        stateStyles.textBoxWidthPercent = wixStyles.textBoxWidthPercent;
    }
    if (canSet('textBoxWidth')) {
        stateStyles.textBoxWidth = wixStyles.textBoxWidth;
    }
    ...
    if (isMobile) {
        for (var key in mobileSpecificDefaultStyles) {
            if (isUndefined(userSavedMobileStyles[key])) {
                stateStyles[key] = mobileSpecificDefaultStyles[key];
            }
        }
    }
    ...
    var getHeightByContent = function(stateStyles) {
        ...
        if (titlePlacement !== GALLERY_v4_CONSTS.placements.SHOW_ABOVE &&
            titlePlacement !== GALLERY_v4_CONSTS.placements.SHOW_BELOW) {
            return 0;
        }
    ...
    if (finalStyleParams.galleryLayout === GALLERY_v4_CONSTS.layout.SLIDESHOW) {
        finalStyleParams.textBoxHeight = finalStyleParams.slideshowInfoSize;
    }
