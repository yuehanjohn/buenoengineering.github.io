...
} from '@wix/wix-json-schema-utils/src/template';
...
var convertToPropertySchema = function(name, property) {
    ...
    var isArray = property.isArray,
        type = property.type,
        restProps = __rest(property, ["isArray", "type"]);
    ...
    return function() {
        ...
        if (config.events) {
            Object.keys(config.events).forEach(function(event) {
                apiStructure.events.push(__assign({
                    name: event
                }, config.events[event]));
            });
        }
        if (config.methods) {
            Object.keys(config.methods).forEach(function(method) {
                apiStructure.functions.push(__assign(__assign({
                    name: method
