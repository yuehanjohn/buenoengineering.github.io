...
import {
    default as slideAnimation
} from '../../settings/options/slideAnimation';
import {
    default as arrowsPosition
} from '../../settings/options/arrowsPosition';
export var calcTargetItemSize = function(options, smartCalc) {
    if (smartCalc === void 0) {
        smartCalc = false;
    }
    if (options.gallerySizeType === GALLERY_SIZE_TYPE.PIXELS &&
        options.gallerySizePx > 0) {
        return options.gallerySizePx;
    } else if (options.gallerySizeType === GALLERY_SIZE_TYPE.RATIO &&
        options.gallerySizeRatio > 0) {
        return (((window && window.innerWidth) || 980) * (options.gallerySizeRatio / 100));
    } else {
        return smartCalc ? smartCalc : options.gallerySize;
    }
};
export var processNumberOfImagesPerRow = function(options) {
    //This will be used in the masonry and grid presets
    var res = __assign({}, options);
    res.fixedColumns = 0;
    if (options.scrollDirection === GALLERY_CONSTS.scrollDirection.VERTICAL || //relevant for grid, in Masonry its fixed to !oneRow
...
export var processGridStyle = function(options) {
    var res = __assign({}, options);
    res.gridStyle =
        res.isVertical ? options.gridStyle : GALLERY_CONSTS.gridStyle.FIT_TO_SCREEN;
    return res;
};
export var processNumberOfImagesPerCol = function(options) {
    //This will be used in the grid preset
    var res = __assign({}, options);
    if (!utils.isUndefined(options.numberOfImagesPerCol) &&
...
var forceInfoOnHoverWhenNeeded = function(options) {
    ...
    if (!GALLERY_CONSTS.isLayout('SLIDER')(_options) && //not slider
...
var forceHoverToShowTextsIfNeeded = function(options) {
    ...
    if (options.EXPERIMENTALallowParallelInfos)
        return _options;
    if (!hasHoverPlacement(_options.titlePlacement) &&
...
var blockScrollOnFadeOrDeckScrollAnimations = function(options) {
    var _options = __assign({}, options);
    if ((options.slideAnimation === GALLERY_CONSTS.slideAnimations.FADE ||
            options.slideAnimation === GALLERY_CONSTS.slideAnimations.DECK) && (slideAnimation.isRelevant(options))) {
...
var blockMouseCursorNavigationOnTouchDevice = function(options) {
    var _options = __assign({}, options);
...
var processImageLoadingWithColorMode = function(options) {
    var _options = __assign({}, options);
    if (_options.imageLoadingMode === LOADING_MODE.COLOR &&
...
var processSpecialGallerySize = function(options) {
    ...
    if (_options.gallerySizeType === GALLERY_SIZE_TYPE.PIXELS &&
        _options.gallerySizePx > 0) {
        _options.targetItemSize = _options.gallerySizePx;
    } else if (_options.gallerySizeType === GALLERY_SIZE_TYPE.RATIO &&
...
var centerArrowsWhenNeeded = function(options) {
    ...
    var filteredPlacement = _options.titlePlacement // filtering hover since it doesn't affect this product
        ...
        .filter(function(placement) {
            return !isHoverPlacement(placement);
        })
        .join(',');
...
export var removeBordersIfNeeded = function(options) {
...
var cropItemsWithCropOnlyFillParam = function(options) {
    var _options = __assign({}, options);
