...
function reverseMigrateOptions(flatOptionsObject) {
    ...
    ///----------- LAYOUT -------------///
    oldOptions = changeNames(oldOptions, __spreadArrays(nameChangedLayoutParams).map(function(ele) {
        return __spreadArrays(ele).reverse();
    }));
    oldOptions = reverseBooleans(oldOptions, __spreadArrays(reversedLayoutParams).map(function(ele) {
        return __spreadArrays(ele).reverse();
    }));
    ...
    ///----------- BEHAVIOUR -------------///
    oldOptions = changeNames(oldOptions, __spreadArrays(nameChangedBehaviourParams).map(function(ele) {
        return __spreadArrays(ele).reverse();
    }));
    oldOptions = reverseBooleans(oldOptions, __spreadArrays(reversedBehaviourParams).map(function(ele) {
        return __spreadArrays(ele).reverse();
    }));
    ...
    ///----------- STYLING -------------///
    oldOptions = changeNames(oldOptions, __spreadArrays(nameChangedStylingParams).map(function(ele) {
        return __spreadArrays(ele).reverse();
    }));
    return oldOptions;
}
...
function process_new_to_old_ThumbnailAlignment(obj) {
    ...
    _obj['galleryThumbnailsAlignment'] = (_a = _obj['galleryThumbnailsAlignment']) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    return _obj;
}
...
function process_new_to_old_CropRatio(obj) {
    ...
    var val = _obj[optionsMap.layoutParams.crop.ratios] && __spreadArrays(_obj[optionsMap.layoutParams.crop.ratios]);
    var newVal = val || [];
    if (newVal.length > 1) {
